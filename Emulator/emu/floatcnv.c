...
int ie_fp(void* p, void* fpp) {
	 ...
	 */
	e = ((ieee[I3] & 0x7f) << 1) | ((ieee[I2] & 0x80) != 0);
...
int fp_ie(void* fpp, void* p) {
	 ...
	 */
	if(e < -126) return FP_ERR;
	if(e > 127) return FP_ERR;
...
int hi_fp(void* p, void* fpp) {
	 ...
	 */
	if((hitech[I2] & 0x80) == 0) return FP_OK;
...
int fp_hi(void* fpp, void* p) {
	 ...
	 */
	if(e < -65) return FP_ERR;
	if(e > 63) return FP_ERR;
...
int am_fp(void* p, void* fpp) {
	 ...
	 * have.
	 */
	if((am9511[I2] & 0x80) == 0) return FP_OK;
 ...
 *     test.
 */
void fp_get(void* fpp, unsigned char* sign, int* exponent, unsigned char* mantissa_h, unsigned int* mantissa_l) {
	struct fp* fptmp = (struct fp*)fpp;
	*sign = fptmp->sign;
...
/* Put into fp.
 */
void fp_put(void* fpp, unsigned char sign, int exponent, unsigned char mantissa_h, unsigned int mantissa_l) {
	struct fp* fptmp = (struct fp*)fpp;
	fptmp->sign = sign;
 ...
 */
size_t fp_size(void) { return sizeof(struct fp); }
